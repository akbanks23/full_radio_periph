...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int time_taken = stop_time - start_time;
	printf("Elapsed time in clocks = %u\n",stop_time-start_time);
		
    // please insert your code here to calculate the actual throughput in Mbytes/second
    // how much data was transferred? How long did it take?
    
	//calculate bytes transferred
	unsigned int bytes_transferred = 2048 * 4; // 2048 reads of 4 bytes each // change obviously, 0
    
	//convert cycles to time in seconds
	float time_spent = (float)time_taken / 125.0e6; //time taken/clk frequency // change obviously, 1
    printf("You transferred %u bytes of data in %f seconds\n",bytes_transferred,time_spent);
    
	//calculate throughput
	float throughput = (float)bytes_transferred / time_spent / (1024 * 1024); //convert bytes to MB
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 7 Alexandria Banks - Custom Peripheral Demonstration\n\r");
